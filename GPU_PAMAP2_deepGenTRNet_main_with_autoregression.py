...
from gen_model.utils.util import log_and_print, matrix_to_string
...
file_name_summary = str(DATASET_NAME) + '_' + str(source_user) + '_' + str(
    target_user) + '_with_autoregression_output_summary.txt'

for Lr_decay1 in [1.0]:  # 1.0, 0.8, 0.5
    for Lr_decay2 in [1.0]:  # 1.0, 0.8, 0.5
        for Optim_Adam_weight_decay in [5e-4]:  # 5e-4, 5e-3, 5e-2, 5e-1
            for Optim_Adam_beta in [0.2]:  # 0.2, 0.5, 0.9
                for lr in [1e-4]:  # 1e-4, 1e-5, 1e-6
                    for Hidden_size in [100]:  # 100, 80, 50
                        for Dis_hidden in [50]:  # 50, 30, 20
                            for ReverseLayer_latent_domain_alpha in [0.1]:  # 0.2, 0.15, 0.25, 0.3, 0.1, 0.35
                                for Variance in [2]:  # 1, 2, 0.7, 3, 0.4, 4, 5
                                    for Num_temporal_states in [2]:  # 2, 3, 4, 5, 6, 7
                                        for temporal_lags in [2]:  # 2, 3, 4, 5, 6, 7
                                            ...
                                            target_acc, cm = GPU_DeepGenTempRela_with_AutoRegression_train(S_torch_loader,
                                                                                                       T_torch_loader,
                                                                                                       ST_torch_loader,
                                                                                                       global_epoch=Epochs,
